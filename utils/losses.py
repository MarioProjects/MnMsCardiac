...
on your application. Pytorch has a number of loss functions <https://pytorch.org/docs/stable/nn.html#loss-functions/>
that you can use out of the box. However, some more advanced and cutting edge loss functions exist that are not (yet)
...
Keep in mind that losses are specific to the type of task. Classification losses are computed differently
from Segmentation losses. Within segmentation domain make sure to use BCE (Binary Cross Entropy) for any work involving
binary masks (e.g. num_classes = 1).
...
Note:
    ...
    - If the model is solving a multi-class classification problem, logits typically become an input
      to the softmax function. The softmax function then generates  vector of (normalized) probabilities
      with one value for each possible class.
For example, BCEWithLogitsLoss is a BCE that accepts R((-inf, inf)) and automatically applies torch.sigmoid
to convert it to ([0,1]) space.

# File Source: https://github.com/achaiah/pywick/blob/master/pywick/losses.py
...
class BCEDiceFocalLoss(nn.Module):
    '''
        ...
        :param gamma: (float,double) gamma > 0 reduces the relative loss for well-classified examples (p>0.5)
                            putting more focus on hard misclassified example
        :param size_average: (bool, optional) By default, the losses are averaged over each loss element in the batch.
        :param weights: (list(), default = [1,1,1]) Optional weighing (0.0-1.0) of the losses. Order [bce, dice, focal]
    '''
    ...
    def forward(self, logits, targets):
        ...
        return self.weights[0] * self.bce(logits, targets) + self.weights[1] * self.dice(logits, targets) + \
               self.weights[2] * self.focal(logits.unsqueeze(1), targets.unsqueeze(1))
...
class FocalLoss3(nn.Module):
    """
        ...
        Params:
            ...
            :param size_average: (bool) - size_average(bool):
...
class ComboBCEDiceLoss(nn.Module):
    ...
    def __init__(self, use_running_mean=False, bce_weight=1, dice_weight=1, eps=1e-6, gamma=0.9,
                 combined_loss_only=True, **kwargs):
        """
        :param use_running_mean: - bool (default: False) Accumulate a running mean and add it to the loss with (1-gamma)
        ...
        :param eps: -
        :param gamma:
        :param combined_loss_only: - bool (default: True) Return a single combined loss or three separate losses
        """
...
class EncNetLoss(nn.CrossEntropyLoss):
    ...
    se_loss is the Semantic Encoding Loss from the paper
    `Context Encoding for Semantic Segmentation <https://arxiv.org/pdf/1803.08904v1>`_.
    ...
    def __init__(self, se_loss=True, se_weight=0.2, nclass=19, aux=False, aux_weight=0.4, weight=None, ignore_index=-1,
                 **kwargs):
...
class MultiTverskyLoss(nn.Module):
    ...
    Args
        ...
        :param weights (Tensor, optional): a manual rescaling weight given to each class. If given, Tensor of size `C`
    """
...
class ActiveContourLoss(nn.Module):
    """
        `Learning Active Contour Models for Medical Image Segmentation: https://shorturl.at/aMQ15
        Note that is only works for B/W masks right now... which is kind of the point of this loss as contours
        in RGB should be cast to B/W before computing the loss.
        Params:
    ...
    def forward(self, logits, target):
        ...
        error_in = probs[:, 0, :, :] * (
                (target[:, 0, :, :] - 1) ** 2)  # invert the ground truth mask and multiply by probs
        ...
        error_out = (probs_diff * target[:, 0, :,
                                  :])  # multiply mask by error, giving us the error terms inside the mask.
...
class HDDTBinaryLoss(nn.Module):
    def __init__(self):
        ...
        Adapted from 3d to 2d from https://github.com/JunMa11/SegLoss/blob/master/test/loss_functions/boundary_loss.py
        """
        super(HDDTBinaryLoss, self).__init__()

    def forward(self, net_output, target):
        """
        net_output: (batch_size, x,y)
...
class AngularPenaltySMLoss(nn.Module):
    ...
    def __init__(self, in_features, out_features, loss_type='arcface', eps=1e-7, s=None, m=None):
        ...
        # loss_type in ['arcface', 'sphereface', 'cosface']
        """
